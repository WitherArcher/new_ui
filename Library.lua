...
local function IsSmallScreen()
    ...
    return ScreenSize.X < 500 or ScreenSize.Y < 500
end
...
function Library:AddToolTip(InfoStr, HoverInstance)
    ...
    HoverInstance.MouseEnter:Connect(function()
        ...
        Tooltip.Position = UDim2.fromOffset(X + 15, Y + 12)
    ...
    HoverInstance.InputBegan:Connect(function(Input)
        if Input.UserInputType == Enum.UserInputType.Touch and not Library:MouseIsOverOpenedFrame() then
            IsHovering = true
            local X, Y = GetInputPosition(Input)
            Tooltip.Position = UDim2.fromOffset(math.clamp(X + 15, 0, UserInputService:GetMouseLocation().X + 100), math.clamp(Y + 12, 0, UserInputService:GetMouseLocation().Y + 100))
            Tooltip.Visible = true
        end
    end)
...
function Library:SetMobileMode(Enable)
    Library.IsMobile = Enable or IsMobile()
    Library.IsSmallScreen = IsSmallScreen()
end

function Library:GetDeviceInfo()
    return {
        IsMobile = Library.IsMobile;
        IsSmallScreen = Library.IsSmallScreen;
    ...
    function Funcs:AddColorPicker(Idx, Info)
        ...
        Library:GiveSignal(InputService.InputBegan:Connect(function(Input)
            if IsPrimaryInput(Input) then
                ...
                if X < AbsPos.X or X > AbsPos.X + AbsSize.X
                    or Y < (AbsPos.Y - 20 - 1) or Y > AbsPos.Y + AbsSize.Y then
        ...
        PickOuter.InputBegan:Connect(function(Input)
            if IsPrimaryInput(Input) and not Library:MouseIsOverOpenedFrame(Input) then
                ...
                Event = InputService.InputBegan:Connect(function(Input)
                    ...
                    elseif IsPrimaryInput(Input) then
                        Key = IsMobile() and 'Touch' or 'MB1';
...
do
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        local TouchPadding = IsMobile() and 4 or 0
        local ToggleSize = IsMobile() and 17 or 13
        ...
        ToggleRegion.InputBegan:Connect(function(Input)
           if IsPrimaryInput(Input) and not Library:MouseIsOverOpenedFrame() then
	...
	if not IsPrimaryInput(Input) or Library:MouseIsOverOpenedFrame() then
		return
	end
	...
	local function UpdateSlider(InputObject)
		local X = GetInputPosition(InputObject)
		local MinX = SliderInner.AbsolutePosition.X
		local MaxX = MinX + Slider.MaxSize
		local NX = math.clamp(X - MinX, 0, Slider.MaxSize)
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        local function RecalculateListPosition()
            ...
            -- Mobile: Adjust dropdown position to avoid going off-screen
            local ScreenSize = GetScreenSize();
            local ListSize = ListOuter.Size
            
            if Y + ListSize.Y.Offset > ScreenSize.Y then
                -- Position above instead of below if not enough space
                Y = DropdownOuter.AbsolutePosition.Y - ListSize.Y.Offset - 1
            end
            
            if X + ListSize.X.Offset > ScreenSize.X then
                -- Shift left if going off-screen right
                X = ScreenSize.X - ListSize.X.Offset - 5
            end
            
            ListOuter.Position = UDim2.fromOffset(math.max(0, X), math.max(0, Y));
        end;
...
function Library:CreateWindow(...)
    ...
    if typeof(Config.Position) ~= 'UDim2' then 
        if IsMobileDevice then
            Config.Position = UDim2.fromOffset(10, 10)
        else
    ...
    Library:GiveSignal(InputService.InputBegan:Connect(function(Input, Processed)
        if type(Library.ToggleKeybind) == 'table' and Library.ToggleKeybind.Type == 'KeyPicker' then
            if Input.UserInputType == Enum.UserInputType.Keyboard and not Processed and Input.KeyCode.Name == Library.ToggleKeybind.Value then
